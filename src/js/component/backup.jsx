...
const Home = () => {
	...
	function addTask(e) {
		...
		if (e.key === 'Enter' || e.type === 'click') {
			if (editId) {
				...
				let resultado = editarTareas(editId, editLabel)
				if(resultado){
					setEditId(null)
					setEditLabel('')
				}else{
					console.log("Algo salio mal en la función editarTareas()")
				}
			} else {
	...
	function handleEdit(e, id, label){
		if (e.key === 'Enter') {
			setEditId(id)
			setEditLabel(label)
			setShowInput(false)
			
			console.log(editLabel, "Soy el label")
							...
							{dataTasks.length === 0 ? (
								...
								dataTasks.map((task, index) => (task !== "" && (
									<li key={task.id} className="border-bottom p-2 px-5 py-3 text-start d-flex justify-content-between align-items-center fw-lighter fs-3 fst-italic" style={{ backgroundColor: isHover === task.id ? 'pink' : 'white', }} onMouseEnter={() => handleMouseEnter(task.id)} onMouseLeave={handleMouseLeave}>
										{showInput ? (
											<div>
												<input value={editLabel} onChange={handleChangeEdit} onKeyDown={(e)=>handleEdit(e, task.id, task.label)} onBlur={handleBlur} onMouseEnter={handleMouseEnterInput} onMouseLeave={handleMouseLeaveInput}></input>
...
// onMouseEnter={()=>handleMouseEnterInput(task.id)}
export default Home;
